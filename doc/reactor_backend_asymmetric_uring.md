...
The `asymmetric_io_uring` backend addresses this by dedicating specific CPUs exclusively to running `io_uring` kernel worker threads and Submission Queue (SQ) polling threads. Shards submit I/O requests directly to `io_uring` instances without performing synchronous system calls, leaving application cores dedicated solely to executing application logic.
...
The `asymmetric_io_uring` backend removes the speculative fast path entirely. All networking and disk I/O requests are consistently submitted via `io_uring` and processed off-core, ensuring shards are never blocked by direct I/O system calls.
...
* **Grouping**: Shards are partitioned into groups, and each group is assigned to a specific worker core.
* **Master Shard**: One shard in each group is designated the master shard. The master shard initializes its `io_uring` instance with the `IORING_SETUP_SQ_AFF` flag and sets `sq_thread_cpu` to the assigned worker core, which spawns the SQ polling thread on that core.
...
1. **Topology Detection**: The backend uses the Portable Hardware Locality (`hwloc`) library to detect NUMA nodes, cores, and SMT sibling threads. If `hwloc` is not available, the backend falls back to a topology-agnostic greedy assignment.
2. **Greedy Allocation**: The backend iterates over the application shards in three passes:
