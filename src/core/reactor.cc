...
reactor_options::reactor_options(program_options::option_group* parent_group)
    ...
    , async_workers_cpuset(*this, "async-workers-cpuset", resource::cpuset{},
                ...
                " Must be a subset of app's cpuset."
    ...
    , uring_buffer_ring_entries(*this, "uring-buffer-ring-entries", std::nullopt, "Count of buffers in the io_uring buffer ring"
                " for reactor backend asymmetric uring. Must be a power of 2 and less or equal to 32768. Specified together with uring-buffer-ring-size."
    ...
    , uring_buffer_ring_size(*this, "uring-buffer-ring-size", std::nullopt, "Buffer size in the io_uring buffer ring"
                " for reactor backend asymmetric uring, in bytes (e.g.: 128k). Specified together with uring-buffer-ring-entries."
                "If omitted, the buffer ring will not be used.")
...
static async_worker_allocation allocate_async_workers(const reactor_backend_selector& backend_selector,
    ...
    if (backend_selector.name() == "asymmetric_io_uring") {
        ...
        // CPUs that are not available are those from --async-workers-cpuset but not present in the cpu_set.
        resource::cpuset not_available_cpus;
        std::set_difference(async_workers_cpu_set.begin(), async_workers_cpu_set.end(),
        ...
        if (!not_available_cpus.empty()) {
            ...
            seastar_logger.error(
                "Bad value for --async-workers-cpuset:{} not allowed. Make sure it is a subset of the app's cpuset. Shutting down.",
                not_available_cpus_list.str());
            exit(1);
        }

        resource::cpuset cpuset_without_workers_cpus;
        std::set_difference(cpu_set.begin(), cpu_set.end(),
                            async_workers_cpu_set.begin(), async_workers_cpu_set.end(),
                            std::inserter(cpuset_without_workers_cpus, cpuset_without_workers_cpus.end()));
        return {async_workers_cpu_set, cpuset_without_workers_cpus};
    }
#endif
...
void smp::configure(const smp_options& smp_opts, const reactor_options& reactor_opts)
{
    ...
    try {
        auto allocation = allocate_async_workers(backend_selector, reactor_opts.async_workers_cpuset.get_value(), cpu_set);
    ...
    if (reactor_opts.uring_buffer_ring_entries && reactor_opts.uring_buffer_ring_size) {
        ...
        if (entries > io_uring_buf_ring_max_entries) {
            seastar_logger.error("io_uring buffer ring entries count must be lower or equal to {}, got {}", io_uring_buf_ring_max_entries, entries);
            exit(1);
        }
