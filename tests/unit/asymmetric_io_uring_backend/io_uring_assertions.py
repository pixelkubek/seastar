...
def assert_expected_io_uring_register_calls(
    ...
    for shard_info in mapping:
        ...
        if is_master_shard:
            ...
            assert io_uring_param.sq_thread in expected_workers_cpus, (
    ...
    assert expected_workers_cpus - used_workers_cpus == set(), (
        f"Not all expected async worker CPUs were used by master shards. Missing: {expected_workers_cpus - used_workers_cpus}"
    )

    assert_even_distribution_of_slave_shards(
...
def assert_master_io_uring_setup_call(
    setup_call: dict, register_calls: list[dict],
) -> io_uring_params:
    EXPECTED_FLAGS = [
        "IORING_SETUP_SQPOLL",
        "IORING_SETUP_SQ_AFF",
...
def assert_even_distribution_of_slave_shards(
    io_uring_params_per_fd: dict[int, io_uring_params],
