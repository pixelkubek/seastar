...
def assert_expected_io_uring_register_calls(
    ...
    mapping,
    files,
    expected_workers_cpus: set[int],
):
    """Assert that the expected io_uring_register calls are present in the strace output files.
    Shards are classified based on number of io_uring_register calls:
    - Master shard: 2 io_uring_register calls (probe and wq_affinity)
    - Slave shard: 1 io_uring_register call (probe)
    ...
    for shard_info in mapping:
        ...
        if is_main_shard:
            if was_main_shard_found:
                raise AssertionError(
                    "Multiple main shards (shard == 0) found in mapping."
                )
            was_main_shard_found = True
        ...
        assert len(register_calls) in [1, 2], (
            f"Each shard should have one or two io_uring_register calls, but shard_id={shard_id} has {len(register_calls)}."
        )

        # Master shard has two io_uring_register calls (probe and wq_affinity), slave shards have one (probe)
        is_master_shard = len(register_calls) == 2
        if is_master_shard:
        ...
        if is_master_shard:
            io_uring_param = assert_master_io_uring_setup_call(
                setup_call, register_calls, expected_workers_cpus
            )
            io_uring_params_per_fd[io_uring_param.fd] = io_uring_param
        else:
            io_uring_param = assert_slave_io_uring_setup_call(
                setup_call, register_calls, io_uring_params_per_fd
            )
            io_uring_params_per_fd[io_uring_param.fd] = io_uring_param

    assert was_main_shard_found, "No main shard (shard_id == 0) found in mapping."
    assert number_of_master_shards > 0, "No master shards found in mapping."
    assert not expected_workers_cpus, (
        f"Some expected worker CPUs were not found in master shards: {expected_workers_cpus}"
    )

    expected_slaves_per_master = max(
        1, number_of_slave_shards // number_of_master_shards
    )
    for io_uring_param in io_uring_params_per_fd.values():
        if io_uring_param.slaves is not None:
...
def _get_syscalls(
    ...
    if is_main_shard:
        setup_calls, register_calls = filter_out_syscalls_for_main_shard(
            setup_calls, register_calls
        )
    return setup_calls, register_calls


def assert_master_io_uring_setup_call(
    setup_call: dict, register_calls: list[dict], expected_workers_cpus: set[int]
) -> io_uring_params:
    EXPECTED_FLAGS = [
        "IORING_SETUP_SQPOLL",
        "IORING_SETUP_SQ_AFF",
    ]
    sq_thread_cpu, wq_fd = assert_io_uring_setup_call(setup_call, EXPECTED_FLAGS)
    ...
    worker_cpu = sq_thread_cpu
    if worker_cpu not in expected_workers_cpus:
        raise AssertionError(
            f"Master shard have worker_cpu {worker_cpu}, which is not in expected async workers cpuset {expected_workers_cpus}."
        )
    expected_workers_cpus.remove(worker_cpu)

    return io_uring_params.master(setup_call["fd"], sq_thread_cpu, wq_affinity[0])


def assert_slave_io_uring_setup_call(
    setup_call: dict,
    ...
    sq_thread_cpu, wq_fd = assert_io_uring_setup_call(setup_call, EXPECTED_FLAGS)
    assert sq_thread_cpu <= 0, (
        "Slave shard io_uring_setup call should have sq_thread_cpu == 0."
    )

    assert wq_fd >= 0, "Slave shard io_uring_setup call has invalid wq_fd."
    if wq_fd not in io_uring_params_per_fd:
        raise AssertionError(
            f"Slave shard io_uring_setup call references unknown wq_fd {wq_fd}."
        )
    io_uring_params_per_fd[wq_fd].add_slave(setup_call["fd"])
...
def assert_io_uring_setup_call(
...
def filter_out_syscalls_for_main_shard(
